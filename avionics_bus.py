""" FEU Alabang - Department of Computer Engineering Final Laboratory Examination: Multi-Sensor Avionics Bus Simulator Target Platform: GitHub Codespaces (Linux / Python 3.11+)
DESCRIPTION: This module simulates a high-frequency avionics sensor bus (MIL-STD-1553/ARINC 429). It polls hardware peripherals concurrently, formats telemetry buffers, and executes downstream signal normalization for flight control algorithms. """
...
if not os.path.exists(CONFIG_FILE):
    ...
    print("Execute: python3 fault_seed.py --id 20231000628.")
    sys.exit(1)
...
class AvionicsPeripheral: 
    ...
    def poll_raw_voltage(self) -> float:
        """Reads physical pin ADC registers. Contract: Must return a floating-point voltage between 0.0V and 5.0V."""
        return 0.0
...
class LiDARPeripheral(AvionicsPeripheral): 
    ...
    def poll_raw_voltage(self) -> float:
        # Simulate clean analog voltage return based on optical bounce
...
class GPSPeripheral(AvionicsPeripheral): 
    ...
    def poll_raw_voltage(self) -> float:
        # Simulate standard 3.3V logic line with minor RF noise
...
class IMUPeripheral(AvionicsPeripheral): 
    ...
    def poll_raw_voltage(self) -> float:
        # Simulates hardware calibration drift injected by student ID seed
        drift_factor = CONFIG.get("imu_voltage_drift", 1.0)
        simulated_voltage = 1.8 * (drift_factor / 5.0)
        
        # When drift exceeds nominal thresholds, hardware flags an internal calibration fault
        if drift_factor > 2.0:
...
class AvionicsBusMaster: 
    ...
    def _async_polling_worker(self, peripheral: AvionicsPeripheral, poll_cycles: int):
        ...
        for _ in range(poll_cycles):
            ...
            # 1. Polymorphic Hardware Polling
            raw_signal = peripheral.poll_raw_voltage()
            
            # Simulate physical bus propagation delay from student seed
            time.sleep(CONFIG.get("bus_race_delay_sec", 0.002))
            
            # 2. Downstream Signal Processing (Quaternion & Voltage Normalization)
            # Standard avionics pipeline scales raw 3.3V logic to 5.0V telemetry reference
            normalized_signal = (raw_signal * 5.0) / 3.3
            
            # 3. Read-Modify-Write to Shared Bus Register
            ...
            # Force thread interleaving during write operations
                time.sleep(0.001)
