""" FEU Alabang - Department of
Computer Engineering Final Laboratory Examination: Multi-Sensor Avionics
Bus Simulator Target Platform: GitHub Codespaces (Linux / Python 3.11+)
DESCRIPTION: This module simulates a high-frequency
avionics sensor bus (MIL-STD-1553/ARINC 429). It polls hardware
peripherals concurrently, formats telemetry buffers, and
executes downstream signal normalization for flight control algorithms. """
import time
import json
import threading
import os
...
if not os.path.exists(CONFIG_FILE):
    ...
    print("Execute: python3 fault_seed.py --id YOUR_STUDENT_ID before running diagnostics.")
    sys.exit(1)
...
class AvionicsBusMaster:
    ...
    def _async_polling_worker(self, peripheral: AvionicsPeripheral, poll_cycles: int):
        ...
        for _ in range(poll_cycles):
            ...
            # Handle dictionary types if IMU returns error_packet instead of float
